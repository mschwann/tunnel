...
class ClientConnector : public SocketInterface {
 public:
  ClientConnector(boost::asio::io_context& io_context, const std::string& ip,
                  size_t port)
  ...
  void connect() {
    s_.getRawSocket()->async_connect(
        boost::asio::ip::tcp::endpoint(ip_address_, port_),
        [this](const boost::system::error_code& ec) { handleConnect(ec); });
  }
