...
struct IpHeader
{
    ...
    uint8_t typeOfService; //3bit for IP precedence, 4bits for Tos, last bit not used.
...
{
    public:
        IpFrame(std::vector<uint8_t>& data):
            data_(data)
        {
...
{
    public:
        MacFrame(std::vector<uint8_t> data)
        : data_(data)
        {
        ...
        void print()
        {
            ...
            std::vector<uint8_t> innerData(std::begin(data_) + sizeof(MacHeader), std::end(data_));
...
{
    public:
        Client(boost::asio::io_context& io_context, const std::string& ip, size_t port, RaiiViFace& vf)
        ...
        bool viface_handler(std::string const& name, uint id, std::vector<uint8_t>& packet) {
