...
class MyDispatcher {
 public:
  ...
  bool handler(std::string const& name, uint id, std::vector<uint8_t>& packet) {
    ...
    SocketRegistry::inst().forEach(
        [&](std::shared_ptr<ServerConnector>& sc) { sc->write(packet); });
    return true;
  }
};
...
class ServerAcceptor {
 public:
  ServerAcceptor(boost::asio::io_service& io_service, short port,
                 RaiiViFace& vf)
  ...
  void doAccept(boost::asio::ip::tcp::socket* s,
                const boost::system::error_code& ec) {
    if (!ec) {
      // Need a better solution, that is not going to waste memory.
      auto sc = std::make_shared<ServerConnector>(
          std::move(std::unique_ptr<boost::asio::ip::tcp::socket>(s)), vf_);
...
void doDispatch(std::set<viface::VIface*> myifaces) {
  ...
  viface::dispatcher_cb mycb =
      std::bind(&MyDispatcher::handler, &printer, std::placeholders::_1,
