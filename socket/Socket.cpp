...
Socket::Socket(std::unique_ptr<boost::asio::ip::tcp::socket> socket, SocketInterface& interface)
...
{
    boost::asio::async_write(*socket_, boost::asio::buffer(data, data.size()), std::bind(&Socket::handleWrite, this, _1));
}

void Socket::read()
{
    socket_->async_read_some(boost::asio::mutable_buffer(buf_.data(), buf_.size()), std::bind(&Socket::handleRead, this, _1, _2));
}
